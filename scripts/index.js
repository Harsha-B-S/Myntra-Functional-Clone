...
function Ratings(stars, noOfReviews) {
  if (stars >= 1) {
    return `${stars} ⭐  | ${
      noOfReviews < 1000 ? noOfReviews : noOfReviews / 1000 + "k"
    }`;
  } else {
    return `<i>Buy! and be the first one to review</i>`;
  }
}
...
function displayItemsOnHomePage() {
  ...
  items.forEach((item) => {
    ...
    <div class="rating">
       ${Ratings(item.rating.stars, item.rating.count)}
    </div>
